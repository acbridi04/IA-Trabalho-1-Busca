...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.startingPosition, (False, False, False, False)) # (posicao, (status dos cantos) -> Nao foram visitados)

    #Goal state: todos os cantos foram visitados (True, True, True, True)
    def isGoalState(self, state):
        return all(state[1])
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            # state = (posicao, (status dos cantos))
            x, y = state[0]
            visitedCorners = list(state[1])  
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                # verifica se o proximo estado é um canto, se for, marca como visitado
                if (nextx, nexty) in self.corners:
                    index = self.corners.index((nextx, nexty)) # retorna qual o indice do canto que foi visitado
...
def cornersHeuristic(state, problem):
    ...
    position, visitationStatus = state
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        #testa se as coordenadas estao na lista de coord com alimentos
